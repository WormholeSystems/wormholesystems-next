<script lang="ts">
	...
	const gestures = $derived(new MapGestures(map));
	...
	// Both the EVE Scout card and the router read `map.eveScout`; one poller feeds them.
	$effect(() => map.startEveScoutPolling());
	...
	$effect(() => connectMapSession(map, tracker, Number(page.url.searchParams.get('system'))));
